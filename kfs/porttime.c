...
klocaltime(long tim, Tm *ct)
{
	...
	copyt = tim - kfs_timezone.minuteswest*60L;
	kgmtime(copyt, ct);
	dayno = ct->yday;
	for(i=0;; i++)
...
kgmtime(long tim, Tm *ct)
{
	 ...
	 */
	if(day >= 0)
		for(d1 = 70; day >= kfs_dysize(d1); d1++)
			day -= kfs_dysize(d1);
	else
		for (d1 = 70; day < 0; d1--)
			day += kfs_dysize(d1-1);
