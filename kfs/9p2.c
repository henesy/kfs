...
static int
fsflush(Chan* chan, Fcall* a, Fcall* b)
{
...
static int
fsclunk(Chan* chan, Fcall* f, Fcall* a)
{
	File *file;
...
static int
fsremove(Chan* chan, Fcall* f, Fcall* a)
{
	File *file;
...
static int
fswstat(Chan* chan, Fcall* f, Fcall* a, char *strs)
{
	Iobuf *p, *p1;
	Dentry *d, *d1, xd;
	File *file;
...
static int
kfs_recv(Chan *c, uchar *buf, int n)
{
	int fd, m, len;
...
static void
kfs_send(Chan *c, uchar *buf, int n)
{
	int fd, m;
...
serve9p2(Chan *chan, uchar *ib, int nib)
{
	...
	for(;;){
		if(nib){
			...
			n = kfs_recv(chan, inbuf, sizeof inbuf);
		if(chat){
